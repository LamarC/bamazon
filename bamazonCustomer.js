      ...
      .then(function (cart) {
        ...
        connection.query('SELECT * FROM products WHERE id= ' + itemID, function (err, selectedItem) {
          ...
          if (quantity <= selectedItem.stock_quantity) {
            ...
            console.log('Thank you for your purchase. Your total is '.yellow)

            let updateQuantity = 'UPDATE products SET stock_quantity = ' + selectedItem.stock_quantity;

            connection.query('UPDATE products SET stock_quantity=? WHERE id=?', [selectedItem.stock_quantity - quantity, itemID],
              function (err, inventory) {
                if (err) throw err;

                selectProduct(); //Returns the prompt to the start point
                //else {
